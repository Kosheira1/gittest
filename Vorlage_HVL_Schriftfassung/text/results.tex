...
\section{Systematic mistake of $\epsilon_{eff}$ compared to $\epsilon_{dielectric}$}
...
The illustration of the height deviation proves in \ref{fig.heightdev} that it is negligible and thus has no significant influence on the results as it is 0.3 per mille for  a height deviation of 0.5 mm. The stair-like shape results again from rounding inaccuracies.  
 \begin{figure}[h!tb]
	\centering
...
To achieve a $\epsilon_{eff}=3.85$, for an electrode distance of 0.15 mm and air gap of 0.5 mm using the
cylindrical electrode geometry, the simulation
suggests the following correction factor:
\begin{equation}
 k_{corr}=\frac{\epsilon_{eff}}{\epsilon_{dielec}}=1.0061
\end{equation}
