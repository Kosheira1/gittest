...
\newline
Different gray points were measured by taking out the sample from the measurement cell and re-inserting in order to estimate how the results vary in that case.
...
And the series of 25 measurements is not enough to predict whether or not and when the measurements are going to converge, so instead of the maximum difference of
...
and the actual dielectric permittivity of the specimen.
...
beneficial to know the extend of this kind of mistake in terms of a correction factor.

In the following, three main factors are introduced that mainly contribute to the systematic mistake.

\subsection{Airgap}
The graph in \ref{fig.airgap} shows the influence or the air gap. The stair-like shape results from rounding inaccuracies caused by COMSOL. There is almost a linear dependency between air gap width and capacitance. 
\begin{figure}[h!tb]
	\centering
...
\begin{equation}
 k_{corr}=\frac{\varepsilon_{eff}}{\varepsilon_{dielec}}=1.0061
\end{equation}
...
\end{figure}
The three data series created from the spectroscopy experiment using the Debye model are shown in the graph \ref{fig.specwithbar} . The solid blue line represents the theoretical  dielectric loss tangent given the parameters of the components in the debye model
...
In order to adequately analyze the data points, graphic \ref{fig.spectroscopy2} shows the previous plot
...
but they also have a larger variance. Which means that one cannot consistently predict a reasonably small range for the mean of said measurement, at least not for only 9 phase averages.
...
Noise floor is a parameter that declares around which value the signal has no power, i.e. noise are located. Generally it can be said that the higher the noise floor is,
...
The following figures illustrate the spectral distribution of the noise for 2 kHz at the output for a sinusoidal input. Using these graphs, the above mentioned value for the noise floor can be determined.
