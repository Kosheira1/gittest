\chapter{Theory}
\section{Part 1}
\section{Derivation of the effective relative permittivity}
...
\subsection{Interdependence between $\angle\epsilon$ and $\epsilon$ \textsubscript{eff}} 
The estimation of the dielectric permittivity. If one wants to estimate the $\epsilon$ of the dielectric material one has to investigate the dependency of magnitude and phase of the $\epsilon$ on the $\epsilon_{eff}$. 
This effect can in general not be neglected as the the simulation for a layered capacitance proves.  
...
\begin{figure}
  ...
  \caption[Kurze Abbildungsbeschreibung]{Interdependency of phase and magnitued of material and effective permittivity for two horizontally stacked dielectrics}
  \label{fig.layeredepsilon}
\end{figure}	
...
We can find the maximum for tan($\delta_{pol}$) by setting the second part of the derivative of loss tangent with
respect to frequency, i.e. \eqref{eq:test} to zero and later find an algebraic equation for the maximum frequency and the maximum value.
...
In order to quantify the desired signal compared to the background noise, the Signal-to-Noise Ratio (SNR) can be measured. The SNF is a the ratio of signal power over signal noise. The power is proportional to the amplitude of the signal.
\begin{equation}
	SNR=\frac{P_{signal}}{P_{noise}} = \left[\frac{(V_{signal})}{(V_{noise})}\right]^2
\end{equation}
The degradation of the SNF is characterized by its input value over its output value, which is the Noise Figure (NF) and is indicated in dB.
\begin{equation}
	NF[dB] = 10log\left(\frac{SNR_{in}}{SNR_{out}}\right)
\end{equation}
